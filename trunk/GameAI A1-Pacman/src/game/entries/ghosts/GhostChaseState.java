...
public class GhostChaseState implements GhostAI {
	int X = 0;
	int Y = 1;
	int pinkDist = 16;
	int tileDist = 4;
	...
	public int[] execute(int ghostType, Game game, long timeDue) {
		...
		if(ghostType == CLYDE){
			double distanceFromPac = MyGhosts.getEuclideanDistance(game.getX(game.getCurGhostLoc(ghostType)), game.getY(game.getCurGhostLoc(ghostType)),  game.getX(game.getCurPacManLoc()), game.getY(game.getCurPacManLoc()));
